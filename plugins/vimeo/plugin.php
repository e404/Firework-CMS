...
class VimeoVideo extends CustomHtmlTag {
	...
	public function __construct() {
		...
		$this->setHandler(function($atts){
			...
			$html = '<div class="video vimeo"><div class="placeholder" style="padding-top:'.$h.'%;"></div><div class="video-wrapper"><iframe id="'.$element_id.'" src="https://player.vimeo.com/video/'.$atts['id'].'?api=1&amp;player_id='.$element_id.($options ? '&amp;'.implode('&amp;',$options) : '').'" frameborder="0"'.($atts['subtitles'] ? ' data-subtitles="'.$atts['subtitles'].'"' : '').' allowfullscreen></iframe></div>';
			if($atts['nointeraction']) {
